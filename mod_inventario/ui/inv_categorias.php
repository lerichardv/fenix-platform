...
$CATEGORIAS = $DB_INV->inv_listado_categorias_semillas();
$cod_categoria = 0;
...
$CATEGORIA = $DB_INV->inv_obtener_info_categoria_simillas($cod_categoria);
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		if (count($CATEGORIA)) {
		?>
			$('#nombre_categoria').val("<?php echo utf8_encode(($CATEGORIA[0]['nombre'])); ?>");
			$('#descripcion_categoria').val("<?php echo utf8_encode($CATEGORIA[0]['descripcion']); ?>");
			$('#cantidad_dias_espera').val("<?php echo $CATEGORIA[0]['cantidadDiasEspera']; ?>");

			// Verificacion sobre el activador
			<?php
			if (
				isset($CATEGORIA[0]['activo'])
				&& $CATEGORIA[0]['activo'] == 1
			) {
			?>
		...
		?>
		codigo_categoria = <?php echo $cod_categoria; ?>;
	...
	$('#btn_guardar').click(function(event) {
		...
		if (error == 0) {
			...
			$('#modal_loading').on('hidden.bs.modal', function() {
				inv_guardar_categoria_semilla(codigo_categoria);
			});
		} else {
	...
	$('.checkbox').change(function(event) {
		...
		inv_cambiar_estado_categoria_semillas($(this).data('id'), ($(this).attr('checked') ? 0 : 1));
	});
</script>
		...
		<div class="panel-body">
			<div class="col-md-12">
				<div class="panel-header">
					<h1 class="translate" data-traducir_english="Category" data-traducir_spanish="Categoría">Categoría</h1>
				</div>
			</div>
			<div class="col-md-12">
				<div class="row">
					<div class="col-md-3">
						<div class="form-group input-group-sm" id="div_nombre_capacitacion">
							<label for="nombre_categoria" class="translate" data-traducir_english="Name" data-traducir_spanish="Nombre">Nombre</label>
							<input type="text" class="form-control letras input requerido requerido_nueva_capacitacion" id="nombre_categoria" name="nombre_categoria">
						</div>
					</div>
					<div class="col-md-3">
						<div class="form-group input-group-sm" id="div_nombre_capacitacion">
							<label for="descripcion_categoria" class="translate" data-traducir_english="Description" data-traducir_spanish="Descripción">Descripción</label>
							<input type="text" class="form-control letras input requerido requerido_nueva_capacitacion" id="descripcion_categoria" name="descripcion_categoria">
						</div>
					</div>
					<div class="col-md-3">
						<div class="form-group input-group-sm">
							<label for="cantidad_dias_espera" class="translate" data-traducir_english="Quantity" data-traducir_spanish="Cantidad de días">Cantidad de días</label>
							<input autocomplete="off" type="number" value="1" min="1" max="100"  class="form-control input requerido_semilla" id="cantidad_dias_espera" name="cantidad_dias_espera">
						</div>
					</div>
					<div class="col-md-3">
						<div class="form-group input-group-sm">
			...
			<div class="col-sm-12">
				<div class="panel panel-primary" style="overflow:auto;">
					<div class="panel-heading panel_cabecera">
						<h3 class="panel-title translate" data-traducir_english="Category List" data-traducir_spanish="Listado de Categorías">Listado de Categorías</h3>
						...
						<table class="table table-condensed display table-striped" id="dev-table">
							<thead>
								<tr class="active info">
									...
									<th width="40%" class="translate" data-traducir_english="Categories" data-traducir_spanish="Categoría">Categoría</th>
									<th width="45%" class="translate" data-traducir_english="Description" data-traducir_spanish="Descripción">Descripción</th>
									<th width="45%" class="translate" data-traducir_english="Quantity of days" data-traducir_spanish="Cantidad de días">Quantity of days</th>
							...
							<tbody id="cuerpo_tabla">
								...
								?>
										<tr class="fila_seleccionable">
											<td onclick="inv_vista_categoria_semillas(<?php echo utf8_encode($categoria['cod_categoria']); ?>)"><?php echo $correlativo; ?></td>
											<td onclick="inv_vista_categoria_semillas(<?php echo utf8_encode($categoria['cod_categoria']); ?>)"><?php echo utf8_encode($categoria['nombre']); ?></td>
											<td onclick="inv_vista_categoria_semillas(<?php echo utf8_encode($categoria['cod_categoria']); ?>)"><?php echo utf8_encode($categoria['descripcion']); ?></td>
											<td onclick="inv_vista_categoria_semillas(<?php echo utf8_encode($categoria['cod_categoria']); ?>)"><?php echo  $categoria['cantidadDiasEspera']; ?></td>
											<td>
												<div class="material-switch pull-right">
													<input class="checkbox" id="checkbox_<?php echo utf8_encode($categoria['cod_categoria']); ?>" data-id="<?php echo utf8_encode($categoria['cod_categoria']); ?>" name="checkbox_<?php echo utf8_encode($categoria['cod_categoria']); ?>" type="checkbox" <?php echo ($categoria['activo'] == 1 ? 'checked="checked"' : ''); ?> />
													<label for="checkbox_<?php echo utf8_encode($categoria['cod_categoria']); ?>" class=""></label>
												</div>
											</td>
										</tr>
								<?php
