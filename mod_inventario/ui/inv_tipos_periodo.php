...
$TIPOS = $DB_INV->inv_listado_tipo_periodos();

$cod_tipo_periodo = $_POST['cod_tipo_periodo'];
if (!isset($_POST['cod_tipo_periodo'])) {
	$cod_tipo_periodo = 0;
}
$TIPO = $DB_INV->inv_obtener_info_tipo_periodo($cod_tipo_periodo);
?>
...
<script type="text/javascript">
	...
	$(document).ready(function() {
	    ...
	    {
	    	?>
	    	$('#tipo_periodo').val("<?php echo utf8_encode($TIPO[0]['tipo_periodo']); ?>");
	    	<?php
	    }
	    ?>
	    codigo_tipo_periodo = <?php echo $cod_tipo_periodo;?>;
	...
	$('#btn_guardar_tipo').click(function(event) {
        ...
        if (error == 0) 
        {
            ...
            $('#modal_loading').on('hidden.bs.modal', function () {            
                inv_guardar_tipo_periodo(codigo_tipo_periodo);
            });
        }
        else
        {
	...
	$('.checkbox').change(function(event) {
		...
		inv_cambiar_estado_tipo_periodo($(this).data('id'),($(this).attr('checked') ? 0 : 1));
	});
</script>
		...
		<div class="panel-body">
			<div class="col-md-12">
				<div class="panel-header">
	                <h1 class="translate" data-traducir_english="Period Type" data-traducir_spanish="Tipo de Periodo">Tipo de Periodo</h1>
	            </div>				
			</div>
            <div class="col-md-12">
            	<div class="row">
	            	<div class="col-md-12">
	            		<div class="form-group input-group-sm" id="div_nombre_capacitacion">
                            <label for="tipo_periodo" class="translate" data-traducir_english="Period Type" data-traducir_spanish="Tipo">Tipo</label>
                            <input type="text"class="form-control letras input requerido requerido_nueva_capacitacion"  id="tipo_periodo" name="tipo_periodo">
                        </div>
	            	</div>
				</div>
            </div>
	    ...
	    <div class="row" style="min-width:100px;">
	        <div class="col-sm-12">
	                ...
	                <div class="panel-heading panel_cabecera"> 
	                        <h3 class="panel-title translate" data-traducir_english="Period Types List" data-traducir_spanish="Listado de Tipos de Periodos">Listado de Tipos de Periodos</h3>		
	                    ...
	                    <table class="table table-condensed display table-striped" id="dev-table" >	                        
	                        <thead>
	                                <tr class="active info">
	                                        ...
	                                        <th width="90%" class="translate" data-traducir_english="Period Type" data-traducir_spanish="Tipo">Tipo</th>
	                        ...
	                        <tbody id="cuerpo_tabla">
	                            ...
	                            if(count($TIPOS))
	                            {
	                            	...
	                            	foreach($TIPOS as $tipo)
	                            	{
	                            		?>
	                            		<tr>
	                            			<td class="hide"><?php echo utf8_encode($tipo['cod_tipo_periodo']);?></td>
	                            			<td onclick="inv_vista_tipo_periodo(<?php echo utf8_encode($tipo['cod_tipo_periodo']);?>)"><?php echo $correlativo;?></td>
	                            			<td onclick="inv_vista_tipo_periodo(<?php echo utf8_encode($tipo['cod_tipo_periodo']);?>)"><?php echo utf8_encode($tipo['tipo_periodo']);?></td>
	                            			<td>	                            				
	                            				<div class="material-switch pull-right">
						                            <input class="checkbox" id="checkbox_<?php echo utf8_encode($tipo['cod_tipo_periodo']);?>" data-id="<?php echo utf8_encode($tipo['cod_tipo_periodo']);?>" name="checkbox_<?php echo utf8_encode($tipo['cod_tipo_periodo']);?>" type="checkbox" <?php echo ($tipo['activo'] == 1 ? 'checked="checked"':'');?>/>
						                            <label for="checkbox_<?php echo utf8_encode($tipo['cod_tipo_periodo']);?>" class=""></label>
						                        </div>
	                            			</td>
	                            		</tr>
	                            		<?php
