...
$RASGOS = $DB_INV->inv_listado_ragos();

$cod_rasgo = $_POST['cod_rasgo'];
if (!isset($_POST['cod_rasgo'])) {
	$cod_rasgo = 0;
}

$RASGO = $DB_INV->inv_obtener_info_rasgo($cod_rasgo);
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		if (count($RASGO)) {
		?>
			$('#nombre_rasgo').val("<?php echo utf8_encode(($RASGO[0]['nombre'])); ?>");
			$('#descripcion_rasgo').val("<?php echo utf8_encode($RASGO[0]['descripcion']); ?>");

			// Verificacion sobre el activador
			<?php
			if (
				isset($RASGO[0]['activo'])
				&& $RASGO[0]['activo'] == 1
			) {
			?>
		...
		?>
		codigo_rasgo = <?php echo $cod_rasgo; ?>;
	...
	$('#btn_guardar').click(function(event) {
		...
		if (error == 0) {
			...
			$('#modal_loading').on('hidden.bs.modal', function() {
				inv_guardar_rasgo(codigo_rasgo);
			});
		} else {
	...
	$('.checkbox').change(function(event) {
		...
		inv_cambiar_estado_rasgos($(this).data('id'), ($(this).attr('checked') ? 0 : 1));
	});
</script>
		...
		<div class="panel-body">
			<div class="col-md-12">
				<div class="panel-header">
					<h1 class="translate" data-traducir_english="Trait" data-traducir_spanish="Rasgo">Rasgo</h1>
				</div>
			</div>
			<div class="col-md-12">
				<div class="row">
					<div class="col-md-3">
						<div class="form-group input-group-sm" id="div_nombre_capacitacion">
							<label for="nombre_rasgo" class="translate" data-traducir_english="Name" data-traducir_spanish="Nombre">Nombre</label>
							<input type="text" class="form-control letras input requerido requerido_nueva_capacitacion" id="nombre_rasgo" name="nombre_rasgo">
						</div>
					</div>
					<div class="col-md-6">
						<div class="form-group input-group-sm" id="div_nombre_capacitacion">
							<label for="descripcion_rasgo" class="translate" data-traducir_english="Description" data-traducir_spanish="Descripción">Descripción</label>
							<input type="text" class="form-control letras input requerido requerido_nueva_capacitacion" id="descripcion_rasgo" name="descripcion_rasgo">
						</div>
					</div>
					<div class="col-md-3">
			...
			<div class="col-sm-12">
				<div class="panel panel-primary" style="overflow:auto;">
					<div class="panel-heading panel_cabecera">
						<h3 class="panel-title translate" data-traducir_english="Trait List" data-traducir_spanish="Listado de Rasgos">Listado de Rasgos</h3>
						...
						<table class="table table-condensed display table-striped" id="dev-table">
							<thead>
								<tr class="active info">
									...
									<th width="40%" class="translate" data-traducir_english="Trait" data-traducir_spanish="Rasgo">Rasgo</th>
							...
							<tbody id="cuerpo_tabla">
								...
								?>
										<tr class="fila_seleccionable">
											<td class="hide"><?php echo utf8_encode($rasgo['cod_variedad']); ?></td>
											<td onclick="inv_vista_rasgos(<?php echo utf8_encode($rasgo['cod_rasgo']); ?>)"><?php echo $correlativo; ?></td>
											<td onclick="inv_vista_rasgos(<?php echo utf8_encode($rasgo['cod_rasgo']); ?>)"><?php echo utf8_encode($rasgo['nombre']); ?></td>
											<td onclick="inv_vista_rasgos(<?php echo utf8_encode($rasgo['cod_rasgo']); ?>)"><?php echo utf8_encode($rasgo['descripcion']); ?></td>
											<td>
												<div class="material-switch pull-right">
													<input class="checkbox" id="checkbox_<?php echo utf8_encode($rasgo['cod_rasgo']); ?>" data-id="<?php echo utf8_encode($rasgo['cod_rasgo']); ?>" name="checkbox_<?php echo utf8_encode($rasgo['cod_rasgo']); ?>" type="checkbox" <?php echo ($rasgo['activo'] == 1 ? 'checked="checked"' : ''); ?> />
													<label for="checkbox_<?php echo utf8_encode($rasgo['cod_rasgo']); ?>" class=""></label>
												</div>
											</td>
										</tr>
								<?php
