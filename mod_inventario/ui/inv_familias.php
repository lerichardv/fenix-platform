...
$FAMILIAS = $DB_INV->inv_listado_familias();
...
$FAMILIA = $DB_INV->inv_obtener_info_familia($cod_familia);
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		if (count($FAMILIA)) {
		?>
			$('#nombre_familia').val("<?php echo utf8_encode(($FAMILIA[0]['nombre'])); ?>");
			$('#descripcion_familia').val("<?php echo utf8_encode($FAMILIA[0]['descripcion']); ?>");

			// Verificacion sobre el activador
			<?php
			if (
				isset($FAMILIA[0]['activo'])
				&& $FAMILIA[0]['activo'] == 1
			) {
			?>
		...
		?>
		codigo_familia = <?php echo $cod_familia; ?>;
	...
	$('#btn_guardar').click(function(event) {
		...
		if (error == 0) {
			...
			$('#modal_loading').on('hidden.bs.modal', function() {
				inv_guardar_familia(codigo_familia);
			});
		} else {
	...
	$('.checkbox').change(function(event) {
		...
		inv_cambiar_estado_familia($(this).data('id'), ($(this).attr('checked') ? 0 : 1));
	});
</script>
		...
		<div class="panel-body">
			<div class="col-md-12">
				<div class="panel-header">
					<h1 class="translate" data-traducir_english="Family" data-traducir_spanish="Familia">Familia</h1>
				</div>
			</div>
			<div class="col-md-12">
				<div class="row">
					<div class="col-md-3">
						<div class="form-group input-group-sm" id="div_nombre_capacitacion">
							<label for="nombre_familia" class="translate" data-traducir_english="Name" data-traducir_spanish="Nombre">Nombre</label>
							<input type="text" class="form-control letras input requerido requerido_nueva_capacitacion" id="nombre_familia" name="nombre_familia">
						</div>
					</div>
					<div class="col-md-6">
						<div class="form-group input-group-sm" id="div_nombre_capacitacion">
							<label for="descripcion_familia" class="translate" data-traducir_english="Description" data-traducir_spanish="Descripción">Descripción</label>
							<input type="text" class="form-control letras input requerido requerido_nueva_capacitacion" id="descripcion_familia" name="descripcion_familia">
						</div>
					</div>
					<div class="col-md-3">
			...
			<div class="col-sm-12">
				<div class="panel panel-primary" style="overflow:auto;">
					<div class="panel-heading panel_cabecera">
						<h3 class="panel-title translate" data-traducir_english="List of families" data-traducir_spanish="Lista de familias">Lista de familias</h3>
						...
						<table class="table table-condensed display table-striped" id="dev-table">
							<thead>
								<tr class="active info">
									...
									<th width="40%" class="translate" data-traducir_english="Family" data-traducir_spanish="Familia">Familia</th>
							...
							<tbody id="cuerpo_tabla">
								...
								?>
										<tr class="fila_seleccionable">
											<td class="hide"><?php echo utf8_encode($familia['cod_variedad']); ?></td>
											<td onclick="inv_vista_familia(<?php echo utf8_encode($familia['cod_familia']); ?>)"><?php echo $correlativo; ?></td>
											<td onclick="inv_vista_familia(<?php echo utf8_encode($familia['cod_familia']); ?>)"><?php echo utf8_encode($familia['nombre']); ?></td>
											<td onclick="inv_vista_familia(<?php echo utf8_encode($familia['cod_familia']); ?>)"><?php echo utf8_encode($familia['descripcion']); ?></td>
											<td>
												<div class="material-switch pull-right">
													<input class="checkbox" id="checkbox_<?php echo utf8_encode($familia['cod_familia']); ?>" data-id="<?php echo utf8_encode($familia['cod_familia']); ?>" name="checkbox_<?php echo utf8_encode($familia['cod_familia']); ?>" type="checkbox" <?php echo ($familia['activo'] == 1 ? 'checked="checked"' : ''); ?> />
													<label for="checkbox_<?php echo utf8_encode($familia['cod_familia']); ?>" class=""></label>
												</div>
											</td>
										</tr>
								<?php
