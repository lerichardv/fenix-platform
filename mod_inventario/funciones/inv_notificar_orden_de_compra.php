...
$codigo_orden     = $_POST['x1'];
$cod_info_empresa = $_POST['x2'];

$USUARIOS = $DB_INV->inv_obtener_listado_usuarios_notificar_orden_compra($cod_info_empresa);

$ORDEN = $DB_INV->inv_obtener_info_orden_compra($codigo_orden);
echo ('0|New order notifications paused');
die();
                                                    ...
                                                    <tbody>
                                                        <tr>
                                                            <td style="word-wrap:break-word;font-size:0px;padding:19px 20px 19px 20px;" align="left">
                                                                <div style="cursor:auto;color:#000000;font-family:Cabin, sans-serif;font-size:15px;line-height:22px;text-align:left;"><h2 style="color: #F05D22; line-height: 100%;"><span style="color:#16a085;">New Purchase Order</span></h2>
                                                                    <p>A new purchase order has been entered for approval. Please go to B&W Farming to check it.</p>
                                                                </div>
                                                            </td>
                                                        </tr>
                                                        <tr>
...
$mail->Subject = 'Notification - New Purchase Order';
...
if(!$mail->send()) 
{
    ...
    echo ('1|Error al notificar ingreso de nueva orden de compra|'. $mail->ErrorInfo);
    //echo ('1|Asignación de documento notificada');
} 
else 
{
    echo ('0|Ingreso de nueva orden de compra notificado');
}