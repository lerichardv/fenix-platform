...
if (isset($fecha_final)) {
	$PLANTACIONES = $DB_REP->rep_reporte_aplicaciones_quimicas($fecha_inicial,$fecha_final);
}
	...
	$(document).ready(function(){
		...
		$( "#excel" ).click(function() {
			...
			if (error != 1) {
				...
				var url = "mod_reportes/funciones/rep_cronograma_aplicaciones_quimicas_excel.php?x1="+x1+"&x2="+x2;
		...
		$('#buscar').click(function(event) {
	        ...
	        if (error == 0) 
	        {
	            ...
	            $('#modal_loading').on('hidden.bs.modal', function () {            
	                $.ajax({
	                	url: 'mod_reportes/ui/rep_cronograma_aplicaciones_quimicas.php',
	                	type: 'POST',
	                	dataType: 'html',
	                	data: {
...
<div class="panel panel-default">
  	<div class="panel-body">
        <div class="page-header">
            <h1 class="translate" data-traducir_english="Chemicals Aplications Chronogram" data-traducir_spanish="Cronograma de Aplicaciones Químicas">Cronograma de Aplicaciones Químicas</h1>
        </div>
		<div class="row">
			<div class="col-md-4">
						...
						<table class="table table-responsive table-condensed table-striped" id="dev-table">
							<thead>
								<tr class="active info">
									...
									<th width="10%" class="translate" data-traducir_english="Tentative Harvest Date" data-traducir_spanish="Fecha Tentativa Cosecha">Fecha Tentativa Cosecha</th>
								</tr>
							</thead>
							<tbody>
	                            <?PHP 
...
<script type="text/javascript">
	...
	/*if (count($PLANTACIONES)) {
		...
		Highcharts.chart('div_grafico', {
		    ...
		    xAxis: {
		        categories: ['Insect(s)', 'Disease(s)', 'Weed(s)', 'Other(s) Damage(s)']
		    },
		    yAxis: {
		        min: 0,
		        title: {
		    ...
		    <?php foreach ($PLANTACIONES as $plantacion) {
		    	echo '{name:"'.utf8_encode($plantacion['anio_plantacion'].'-'.$plantacion['num_plantacion'].'-'.
		    		$plantacion['codigo_temporada']).'", data: ['.$plantacion['insectos'].','.$plantacion['enfermedades'].','.$plantacion['hierbas'].','.
		    		$plantacion['otros_danos'].']},';
		    	}
		    	?>
		    ]
		});
