...
if (isset($fecha_final)) {
	//$PLANTACIONES 	= $DB_REP->rep_reporte_plantaciones_tierra($fecha_inicial,$fecha_final,$cod_granjas_usuario);
	//$TOTALES 		= $DB_REP->rep_reporte_plantaciones_tierra_totales($fecha_inicial,$fecha_final,$cod_granjas_usuario);
	$PLANTACIONES 	= $DB_REP->rep_reporte_plantaciones_tierra($fecha_inicial,$fecha_final,$cod_info_empresa);
	$TOTALES 		= $DB_REP->rep_reporte_plantaciones_tierra_totales($fecha_inicial,$fecha_final,$cod_info_empresa);
}
	...
	$(document).ready(function(){
		        ...
		        "buttons": [
		            {
		                ...
		                pageSize: 'A0',
		                title: 'Planting Land Crops Report',
		            },
		            'print',
		        ]
	        });
		...
		$( "#excel" ).click(function() {
			...
			if (error != 1) {
				...
				var url = "mod_reportes/funciones/rep_reporte_plantaciones_tierra_excel.php?x1="+x1+"&x2="+x2+"&x3="+x3;
		...
		$('#buscar').click(function(event) {
	        ...
	        if (error == 0)
	        {
	            ...
	            $('#modal_loading').on('hidden.bs.modal', function () {
	                $.ajax({
	                	url: 'mod_reportes/ui/rep_reporte_plantaciones_tierra.php',
	                	type: 'POST',
	                	dataType: 'html',
	                	data: {
...
<div class="panel panel-default">
  	<div class="panel-body">
        <div class="page-header">
            <h1 class="translate" data-traducir_english="Planting Records Land Crops Report" data-traducir_spanish="Reporte de Cultivos de Tierra">Reporte de Cultivos de Tierra</h1>
        </div>
		<div class="row">
			<div class="col-md-3">
						...
						<table class="table table-responsive table-condensed table-striped" id="dev-table">
							<thead>
								<tr class="active info">
									...
									<th width="10%" class="translate" data-traducir_english="Blocks Range" data-traducir_spanish="Ranago Bloques">Ranago Bloques</th>
									<th width="10%" class="translate" data-traducir_english="Acres" data-traducir_spanish="Acres">Acres</th>
									<th width="10%" class="translate" data-traducir_english="Total Acres" data-traducir_spanish="Total Acres">Total Acres</th>
									<th width="10%" class="translate" data-traducir_english="Seeds" data-traducir_spanish="Semillas">Semillas</th>
									<th width="15%" class="translate" data-traducir_english="Amount Seed" data-traducir_spanish="Cantidad Semillas">Cantidad Semillas</th>
									<th width="10%" class="translate" data-traducir_english="Lot No." data-traducir_spanish="Lote Nro.">Lote Nro.</th>
									<th width="10%" class="translate" data-traducir_english="Machinery Code" data-traducir_spanish="Código Maquinaria">Código Maquinaria</th>
	                            ...
	                            <?PHP
	                            	...
	                            	if (count($PLANTACIONES)) {
	                            		...
	                            		foreach ($PLANTACIONES as $plantacion) {
	                            			...
	                            			$cuerpo_tabla .= '<tr>
	                            									<td>'.$correlativo.'</td>
	                            									<td>'.utf8_encode($plantacion['gerencia']).'</td>
	                            									...
	                            									<td>'.utf8_encode($plantacion['fecha_plantacion_planeada']).'</td>
	                            									...
	                            									<td>'.utf8_encode($plantacion['acres']).'</td>
	                            									<td>'.utf8_encode($plantacion['total_acres']).'</td>
	                            									<td>'.utf8_encode($plantacion['semillas']).'</td>
	                            									<td>'.utf8_encode($plantacion['cantidad_semillas']).'</td>
	                            									<td>'.utf8_encode($plantacion['lotes']).'</td>
	                            									<td>'.utf8_encode($plantacion['codigos_maquinaria']).'</td>
...
<script type="text/javascript">
	...
	/*if (count($TOTALES)) {
		...
		Highcharts.chart('div_grafico', {
		    ...
		    title: {
		        text: 'Planting Records Land Crops Report'
		    },
		    subtitle: {
		        text: 'Planting List with Land Crops Records'
		    },
		    xAxis: {
		        type: 'category',
		        labels: {
		    ...
		    tooltip: {
		        pointFormat: 'Acres: <b>{point.y:.1f} acres</b>'
		    },
		    series: [{
		        name: 'Planting',
		        data: [
		        	...
		        	foreach ($TOTALES as $plantacion) {
		        		echo '["'.utf8_encode($plantacion['anio_plantacion'].'-'.$plantacion['num_plantacion'].'-'.$plantacion['codigo_temporada']).'",'.$plantacion['total_acres'].'],';
		        	}
		        	?>
		        ],
		        dataLabels: {
