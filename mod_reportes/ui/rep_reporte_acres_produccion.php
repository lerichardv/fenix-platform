	...
	$(document).ready(function(){
		        ...
		        "buttons": [
		            {
		                ...
		                pageSize: 'A0',
		                title: 'In Production Acres Report',
		            },
		            'print',
		        ]
	        });
		...
		$( "#excel" ).click(function() {
			...
			if (error != 1) {
				...
				var url = "mod_reportes/funciones/rep_reporte_acres_produccion_excel.php?x1="+x1+"&x2="+x2+"&x3="+x3;
		...
		$('#buscar').click(function(event) {
	        ...
	        if (error == 0)
	        {
	            ...
	            $('#modal_loading').on('hidden.bs.modal', function () {
	                $.ajax({
	                	url: 'mod_reportes/ui/rep_reporte_acres_produccion.php',
	                	type: 'POST',
	                	dataType: 'html',
	                	data: {
...
<div class="panel panel-default">
  	<div class="panel-body">
        <div class="page-header">
            <h1 class="translate" data-traducir_english="Acres in Production Report" data-traducir_spanish="Reporte de Acres en Producción">Reporte de Acres en Producción</h1>
        </div>
		<div class="row">
			<div class="col-md-3">
						...
						<table class="table table-responsive table-condensed table-striped" id="dev-table">
							<thead>
								<tr class="active info">
									...
									<th width="10%" class="translate" data-traducir_english="Block" data-traducir_spanish="Bloque">Bloque</th>
									<th width="10%" class="translate" data-traducir_english="Date Change Stage" data-traducir_spanish="Fecha Cambio Estado">Fecha Cambio Estado</th>
									<th width="10%" class="translate" data-traducir_english="Acres" data-traducir_spanish="Acres">Acres</th>
									<th width="10%" class="translate" data-traducir_english="Blocks Planted" data-traducir_spanish="Bloques Plantado">Bloques Plantado</th>
									<th width="15%" class="translate" data-traducir_english="Blocks Trimmed" data-traducir_spanish="Bloques Podado">Bloques Podado</th>
									<th width="10%" class="translate" data-traducir_english="Blocks Growing" data-traducir_spanish="Bloques Crecimiento">Bloques Crecimiento</th>
									<th width="10%" class="translate" data-traducir_english="Blocks Ready to Harvest" data-traducir_spanish="Bloques Listo para Cosecha">Bloques Listo para Cosecha</th>
									<th width="10%" class="translate" data-traducir_english="Blocks Harvested" data-traducir_spanish="Bloques Cosechado">Bloques Cosechado</th>
	                            	...
	                            	if (count($PLANTACIONES)) {
	                            		...
	                            		foreach ($PLANTACIONES as $plantacion) {
	                            			...
	                            			$cuerpo_tabla .= '<tr>
	                            									...
	                            									<td>'.utf8_encode($plantacion['date_update']).'</td>
	                            									<td>'.utf8_encode($plantacion['num_acres']).'</td>
	                            									<td>'.utf8_encode($plantacion['plantados']).'</td>
	                            									<td>'.utf8_encode($plantacion['podados']).'</td>
	                            									<td>'.utf8_encode($plantacion['crecimiento']).'</td>
	                            									<td>'.utf8_encode($plantacion['listo_cosechar']).'</td>
	                            									<td>'.utf8_encode($plantacion['cosechado']).'</td>
	                            									<td>'.$fecha.'</td>
	                            									<td>'.$hora.'</td>'
	                            							.'</tr>';
	                       					$correlativo++;
...
<script type="text/javascript">
	...
	/*if (count($PLANTACIONES)) {
		...
		Highcharts.chart('div_grafico', {
		    ...
		    title: {
		        text: 'Acres in Production Report'
		    },
		    xAxis: {
		        categories: ['Planted', 'Trimmed', 'Growing', 'Ready to Harvest', 'Harvested']
		    },
		    yAxis: {
		        min: 0,
		        title: {
		    ...
		    <?php foreach ($PLANTACIONES as $plantacion) {
		    	echo '{name:"'.utf8_encode($plantacion['anio_plantacion'].'-'.$plantacion['num_plantacion'].'-'.
		    		$plantacion['codigo_temporada']).'", data: ['.$plantacion['cantidad_plantados'].','.$plantacion['cantidad_podados'].','.$plantacion['cantidad_crecimiento'].','.
		    		$plantacion['cantidad_listo_cosechar'].','.$plantacion['cantidad_cosechados'].']},';
		    	}
		    	?>
		    ]
		});
