	...
	$(document).ready(function(){
		        ...
		        "buttons": [
		            {
		                ...
		                customize : function(doc){
				            ...
				            $('#dev-table').find('tbody tr:first-child td').each(function(){
		...
		$( "#excel" ).click(function() {
			...
			if (error != 1) {
				...
				var url = "mod_reportes/funciones/rep_reporte_cosecha_excel.php?x1="+x1+"&x2="+x2+"&x3="+x3;
		...
		$('#buscar').click(function(event) {
	        ...
	        if (error == 0)
	        {
	            ...
	            $('#modal_loading').on('hidden.bs.modal', function () {
	                $.ajax({
	                	url: 'mod_reportes/ui/rep_reporte_cosecha.php',
	                	type: 'POST',
	                	dataType: 'html',
	                	data: {
...
<div class="panel panel-default">
  	<div class="panel-body">
        <div class="page-header">
            <h1 class="translate" data-traducir_english="Harvesting Report" data-traducir_spanish="Reporte de Cosecha">Reporte de Cosecha</h1>
        </div>
		<div class="row">
			<div class="col-md-3">
						...
						<table class="table table-responsive table-condensed table-striped" id="dev-table">
							<thead>
								<tr class="active info">
									...
									<th width="15%" class="translate" data-traducir_english="Harvest Date" data-traducir_spanish="Fecha Cosecha">Fecha Cosecha</th>
									...
									<th width="15%" class="translate" data-traducir_english="Seeds" data-traducir_spanish="Semillas">Semillas</th>
									...
									<th width="10%" class="translate" data-traducir_english="Blocks Range" data-traducir_spanish="Rango Bloques">Rango Bloques</th>
									<th width="10%" class="translate" data-traducir_english="Total Blocks" data-traducir_spanish="Total Bloques">Total Bloques</th>
									<th width="10%" class="translate" data-traducir_english="PHI" data-traducir_spanish="PHI">PHI</th>
									<th width="15%" class="translate" data-traducir_english="Loose" data-traducir_spanish="Suelto">Suelto</th>
									<th width="10%" class="translate" data-traducir_english="Increment (Bunch-Loose)" data-traducir_spanish="Incremento (Manojo-Suelto)">Incremento (Manojo-Suelto)</th>
									<th width="10%" class="translate" data-traducir_english="Area Finished" data-traducir_spanish="Area Finalizada">Area Finalizada</th>
									<th width="10%" class="translate" data-traducir_english="Acres Harvested" data-traducir_spanish="Acres Cosechados">Acres Cosechados</th>
									<th width="10%" class="translate" data-traducir_english="PO#" data-traducir_spanish="PO#">PO#</th>
									<th width="10%" class="translate" data-traducir_english="Harvested Quantity" data-traducir_spanish="Cantidad Cosechada">Cantidad Cosechada</th>
									<th width="10%" class="translate" data-traducir_english="Packaged Quantity" data-traducir_spanish="Cantidad Empacada">Cantidad Empacada</th>
									<th width="10%" class="translate" data-traducir_english="Packaged Date" data-traducir_spanish="Fecha Empacada">Fecha Empacada</th>
									<th width="10%" class="translate" data-traducir_english="Totes Harvested" data-traducir_spanish="Bolsas Cosechadas">Bolsas Cosechadas</th>
									<th width="10%" class="translate" data-traducir_english="Totes Packed" data-traducir_spanish="Bolsas Empacadas">Bolsas Empacadas</th>
	                            ...
	                            <?PHP
	                            	...
	                            	if (count($PLANTACIONES)) {
	                            		...
	                            		foreach ($PLANTACIONES as $plantacion) {
	                            			...
	                            			$cuerpo_tabla .= '<tr>
	                            									<td>'.$correlativo.'</td>
	                            									<td>'.utf8_encode($plantacion['harvest_date']).'</td>
	                            									...
	                            									<td>'.utf8_encode($plantacion['semillas']).'</td>
	                            									<td>'.utf8_encode($plantacion['zonas']).'</td>
	                            									<td>'.utf8_encode($plantacion['bloques']).'</td>
	                            									<td>'.utf8_encode($plantacion['min_bloque'].'-'.$plantacion['max_bloque']).'</td>
	                            									<td>'.utf8_encode($plantacion['cantidad_bloques']).'</td>
	                            									<td>'.utf8_encode($plantacion['phi'] == 1 ? 'Yes':'No').'</td>
	                            									<td>'.utf8_encode($plantacion['loose'] == 1 ? 'Yes':'No').'</td>
	                            									<td>'.utf8_encode($loose_bunches[$plantacion['increment_bunch_cello']]).'</td>
	                            									<td>'.utf8_encode($plantacion['area_finished'] == 1 ? 'Yes':'No').'</td>
	                            									<td>'.utf8_encode($plantacion['acres_harvested']).'</td>
	                            									<td>'.utf8_encode($plantacion['orden_compra']).'</td>
	                            									<td>'.utf8_encode($plantacion['cantidad_cosechada']).'</td>
	                            									<td>'.utf8_encode($plantacion['cantidad_empacada']).'</td>
	                            									<td>'.utf8_encode($plantacion['date_packed']).'</td>
	                            									<td>'.utf8_encode($plantacion['totes_harvested']).'</td>
	                            									<td>'.utf8_encode($plantacion['totes_packed']).'</td>
	                            									<td>'.utf8_encode($plantacion['commments_harvesting_worksheet']).'</td>
	                            									<td>'.$fecha.'</td>
	                            									<td>'.$hora.'</td>'
	                            							.'</tr>';
	                       					$correlativo++;
...
<script type="text/javascript">
	...
	/*if (count($PLANTACIONES)) {
		...
		Highcharts.chart('div_grafico', {
		    ...
		    title: {
		        text: 'Harvesting Planting Report'
		    },
		    subtitle: {
		        text: 'Planting List'
		    },
		    xAxis: {
		        categories: ['Acres Harvested', 'Harvested Quantity', 'Packaged Quantity'],
		        title: {
		            text: null
		        }
		    },
		    ...
		    <?php
        	foreach ($PLANTACIONES as $plantacion) {
        		echo '{name: "'.utf8_encode($plantacion['anio_plantacion'].'-'.$plantacion['num_plantacion'].'-'.$plantacion['codigo_temporada']).'", data: ['.($plantacion['acres_harvested'] == NULL ? 0:$plantacion['acres_harvested']).','.($plantacion['cantidad_cosechada'] == NULL ? 0:$plantacion['cantidad_cosechada']).','.($plantacion['cantidad_empacada'] == NULL ? 0:$plantacion['cantidad_empacada']).']},';
        	}
        	?>]
		});
		<?php
