...
if (isset($fecha_final)) {
	...
	$PLANTACIONES = $DB_REP->rep_reporte_costos($fecha_inicial, $fecha_final, $cod_info_empresa);
	// $PLANTACIONES = [];
}
	...
	$(document).ready(function() {
		...
		$("#excel").click(function() {
			...
			if (error != 1) {
				...
				var url = "mod_reportes/funciones/rep_reporte_costos_excel.php?x1=" + x1 + "&x2=" + x2 + "&x3=" + x3;
		...
		$("#pdf").click(function() {
			...
			if (error != 1) {
				...
				//x3 = $("#estado").val();
				//x4 = $("#tipo_documento").val();
				var url = "mod_reportes/funciones/rep_reporte_costos_pdf.php?x1=" + x1 + "&x2=" + x2;
				$(location).attr('href', url);
			} else {
				grl_mensaje('You must fill the empty fields', 'Debe llenar los campos vacíos', 'warning');
			}
		...
		$('#buscar').click(function(event) {
			...
			if (error == 0) {
				...
				$('#modal_loading').on('hidden.bs.modal', function() {
					$.ajax({
							url: 'mod_reportes/ui/rep_reporte_costos.php',
							type: 'POST',
							dataType: 'html',
							data: {
...
<div class="panel panel-default">
	<div class="panel-body">
		<div class="page-header">
			<h1 class="translate" data-traducir_english="Planting Costs Report" data-traducir_spanish="Reporte de Costos de las Plantaciones">Reporte de Costos de las Plantaciones</h1>
		</div>
		<div class="row">
			<div class="col-md-3">
						...
						<table class="table table-responsive table-condensed table-striped" id="dev-table">
							<thead>
								<tr class="active info">
									...
									<th width="10%" class="translate" data-traducir_english="Chemical Type" data-traducir_spanish="Tipo de Químico">Tipo de Químico</th>
									...
									<th width="10%" class="translate" data-traducir_english="Applied Amount" data-traducir_spanish="Cantidad Aplicada">Cantidad Aplicada</th>
									<th width="10%" class="translate" data-traducir_english="Costs" data-traducir_spanish="Costos">Costos</th>
								...
								if (count($PLANTACIONES)) {
									...
									foreach ($PLANTACIONES as $plantacion) {
										...
										$cuerpo_tabla .= '<tr>
	                            									...
	                            									<td>' . utf8_encode($plantacion['quimicos']) . '</td>
	                            									<td>' . utf8_encode($plantacion['unidades']) . '</td>
	                            									<td>' . utf8_encode($plantacion['cantidad_aplicada']) . '</td>
	                            									<td>' . utf8_encode($plantacion['costos']) . '</td>
	                            									<td>' . utf8_encode($plantacion['total_costos']) . '</td>
	                            									<td>' . $fecha . '</td>
	                            									<td>' . $hora . '</td>'
											. '</tr>';
										$correlativo++;
...
<script type="text/javascript">
	...
	/*if (count($PLANTACIONES)) {
		...
		Highcharts.chart('div_grafico', {
		    ...
		    subtitle: {
		        text: 'Planting List with its Costs'
		    },
		    xAxis: {
		        type: 'category',
		        labels: {
		    ...
		    tooltip: {
		        pointFormat: 'Costs: <b>$ {point.y:.1f}</b>'
		    },
		    series: [{
		        name: 'Planting',
		        data: [
		        	<?php
		        	foreach ($PLANTACIONES as $plantacion) {
		        		echo '["'.utf8_encode($plantacion['anio_plantacion'].'-'.$plantacion['num_plantacion'].'-'.$plantacion['codigo_temporada']).'",'.$plantacion['total_costos'].'],';
		        	}
		        	?>
		        ],
		        dataLabels: {
