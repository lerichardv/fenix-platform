<?PHP
/*
 * Reporte de inventario de semillas.
	...
	$(document).ready(function(){
		...
		$( "#excel" ).click(function() {
			...
			if (error != 1) {
				...
				var url = "mod_reportes/funciones/rep_reporte_inventario_semillas_excel.php?x1="+x1+"&x2="+x2+"&x3="+x3;
		...
		$('#buscar').click(function(event) {
	        ...
	        if (error == 0)
	        {
	            ...
	            $('#modal_loading').on('hidden.bs.modal', function () {
	                $.ajax({
	                	url: 'mod_reportes/ui/rep_reporte_inventario_semillas.php',
	                	type: 'POST',
	                	dataType: 'html',
	                	data: {
...
<div class="panel panel-default">
  	<div class="panel-body">
        <div class="page-header">
            <h1 class="translate" data-traducir_english="Seeds Inventories Report" data-traducir_spanish="Reporte de Inventario de Semillas">Reporte de Inventario de Semillas</h1>
        </div>
		<div class="row">
			<div class="col-md-3">
						...
						<table class="table table-responsive table-condensed table-striped" id="dev-table">
							<thead>
								<tr class="active info">
									...
									<th width="10%" class="translate" data-traducir_english="Variety" data-traducir_spanish="Variedad">Variedad</th>
									<th width="10%" class="translate" data-traducir_english="Seed" data-traducir_spanish="Semilla">Semilla</th>
									...
									<th width="10%" class="translate" data-traducir_english="Planted" data-traducir_spanish="Plantado">Plantado</th>
							...
							<tbody>
	                            <?PHP
	                            	...
	                            	if (count($PLANTACIONES)) {
	                            		...
	                            		foreach ($PLANTACIONES as $plantacion) {
	                            			...
	                            			$cuerpo_tabla .= '<tr>
	                            									...
	                            									<td>'.utf8_encode($plantacion['codigo_semilla']).'</td>
	                            									<td>'.utf8_encode($plantacion['variedad_semilla']).'</td>
	                            									<td>'.utf8_encode($plantacion['nombre_semilla']).'</td>
	                            									...
	                            									<td>'.$entro.'</td>
	                            									<td>'.$salio.'</td>
	                            									<td>'.floatval($monto_inicial-$plantado+$recibido+$entro-$salio+$sumado-$restado).'</td>
	                            									<td>'.$sumado.'</td>
	                            									<td>'.$restado.'</td>
	                            									<td>'.($monto_inicial-$plantado+$recibido+$entro-$salio+$sumado-$restado).'</td>'
	                            							.'</tr>';
	                       					$correlativo++;
	                            		}
	                                ...
	                                { //Si no hay registros informará al diligencia
	                                    $cuerpo_tabla .= '<tr>
																<td align="center" colspan="15">No records</td>
	                                                      </tr>';
	                                }
	                                echo $cuerpo_tabla;
	                            ?>
