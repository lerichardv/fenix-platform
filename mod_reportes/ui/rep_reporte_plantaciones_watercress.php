...
if (isset($fecha_final)) {
	...
	//$TOTALES 		= $DB_REP->rep_reporte_plantaciones_watercress_totales($fecha_inicial,$fecha_final,$cod_granjas_usuario);
	$PLANTACIONES 	= $DB_REP->rep_reporte_plantaciones_watercress($fecha_inicial,$fecha_final,$cod_info_empresa);
	$TOTALES 		= $DB_REP->rep_reporte_plantaciones_watercress_totales($fecha_inicial,$fecha_final,$cod_info_empresa);
}
	...
	$(document).ready(function(){
		        ...
		        "buttons": [
		            {
		                ...
		                pageSize: 'A0',
		                title: 'Planting Watercress Report',
		            },
		            'print',
		        ]
	        });
		...
		$( "#excel" ).click(function() {
			...
			if (error != 1) {
				...
				var url = "mod_reportes/funciones/rep_reporte_plantaciones_watercress_excel.php?x1="+x1+"&x2="+x2+"&x3="+x3;
		...
		$('#buscar').click(function(event) {
	        ...
	        if (error == 0)
	        {
	            ...
	            $('#modal_loading').on('hidden.bs.modal', function () {
	                $.ajax({
	                	url: 'mod_reportes/ui/rep_reporte_plantaciones_watercress.php',
	                	type: 'POST',
	                	dataType: 'html',
	                	data: {
...
<div class="panel panel-default">
  	<div class="panel-body">
        <div class="page-header">
            <h1 class="translate" data-traducir_english="Planting Records Watercress Report" data-traducir_spanish="Reporte de Cultivos de Berro de Agua">Reporte de Cultivos de Berro de Agua</h1>
        </div>
		<div class="row">
			<div class="col-md-3">
						...
						<table class="table table-responsive table-condensed table-striped" id="dev-table">
							<thead>
								<tr class="active info">
									...
									<th width="15%" class="translate" data-traducir_english="Company Name" data-traducir_spanish="Gerencia">Gerencia</th>
									<th width="15%" class="translate" data-traducir_english="Planting Send" data-traducir_spanish="Plantación Envía">Plantación Envía</th>
									<th width="15%" class="translate" data-traducir_english="Greenhouse" data-traducir_spanish="Invernadero">Invernadero</th>
									<!-- <th width="15%" class="translate" data-traducir_english="Planting Date" data-traducir_spanish="Fecha Plantación">Fecha Plantación</th> -->
									...
									<th width="15%" class="translate" data-traducir_english="Company Name" data-traducir_spanish="Gerencia">Gerencia</th>
									<th width="15%" class="translate" data-traducir_english="Planting Receive" data-traducir_spanish="Plantación Recibe">Plantación Recibe</th>
									<th width="15%" class="translate" data-traducir_english="Greenhouse" data-traducir_spanish="Invernadero">Invernadero</th>
									<th width="15%" class="translate" data-traducir_english="Planning Date" data-traducir_spanish="Fecha Planificación">Fecha Planificación</th>
									<th width="10%" class="translate" data-traducir_english="Zones" data-traducir_spanish="Zonas">Zonas</th>
									<th width="10%" class="translate" data-traducir_english="Blocks" data-traducir_spanish="Bloques">Bloques</th>
									<th width="10%" class="translate" data-traducir_english="Acres" data-traducir_spanish="Acres">Acres</th>
									<th width="10%" class="translate" data-traducir_english="Observations" data-traducir_spanish="Observaciones">Observaciones</th>
									<th width="10%" class="translate" data-traducir_english="User Name" data-traducir_spanish="Usuario">Usuario</th>
	                            ...
	                            <?PHP
	                            	...
	                            	if (count($PLANTACIONES)) {
	                            		...
	                            		foreach ($PLANTACIONES as $plantacion) {
	                            			...
	                            			$cuerpo_tabla .= '<tr>
	                            									<td>'.$correlativo.'</td>
	                            									<td>'.utf8_encode($plantacion['gerencia_envia']).'</td>
	                            									<td>'.utf8_encode($plantacion['anio_plantacion_envia'].'-'.$plantacion['num_plantacion_envia'].'-'.$plantacion['codigo_temporada_envia']).'</td>
	                            									<td>'.utf8_encode($plantacion['nombre_empresa_envia']).'</td>
	                            									<!--<td>'.utf8_encode($plantacion['fecha_planeada_envia']).'</td>-->
	                            									<td>'.utf8_encode($plantacion['zonas_envia']).'</td>
	                            									<td>'.utf8_encode($plantacion['bloques_envia']).'</td>
	                            									<td>'.utf8_encode($plantacion['total_acres_envia']).'</td>
	                            									<td>'.utf8_encode($plantacion['gerencia_recibe']).'</td>
	                            									<td>'.utf8_encode($plantacion['anio_plantacion_recibe'].'-'.$plantacion['num_plantacion_recibe'].'-'.$plantacion['codigo_temporada_recibe']).'</td>
	                            									<td>'.utf8_encode($plantacion['nombre_empresa_recibe']).'</td>
	                            									<td>'.utf8_encode($plantacion['fecha_trasplante']).'</td>
	                            									<td>'.utf8_encode($plantacion['zonas_recibe']).'</td>
	                            									<td>'.utf8_encode($plantacion['bloques_recibe']).'</td>
	                            									<td>'.utf8_encode($plantacion['total_acres_recibe']).'</td>
	                            									<td>'.utf8_encode($plantacion['observacion']).'</td>
	                            									<td>'.utf8_encode($plantacion['nombre_usuario']).'</td>
	                            									<td>'.$fecha.'</td>
	                            									<td>'.$hora.'</td>'
	                            							.'</tr>';
	                       					$correlativo++;
...
<script type="text/javascript">
	...
	/*if (count($TOTALES)) {
		...
		Highcharts.chart('div_grafico', {
		    ...
		    title: {
		        text: 'Planting Records Watercress Report'
		    },
		    subtitle: {
		        text: 'Planting List with Watercress Records'
		    },
		    xAxis: {
		        type: 'category',
		        labels: {
		    ...
		    tooltip: {
		        pointFormat: 'Acres: <b>{point.y:.2f} acres</b>'
		    },
		    series: [{
		        name: 'Planting',
		        data: [
		        	<?php
		        	foreach ($TOTALES as $plantacion) {
		        		echo '["'.utf8_encode($plantacion['anio_plantacion_recibe'].'-'.$plantacion['num_plantacion_recibe'].'-'.$plantacion['codigo_temporada_recibe']).'",'.$plantacion['total_acres_recibe'].'],';
		        	}
		        	?>
		        ],
		        dataLabels: {
