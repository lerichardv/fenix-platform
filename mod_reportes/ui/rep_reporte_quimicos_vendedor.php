...
if (isset($fecha_final)) {
	$PROVEEDORES = $DB_REP->rep_reporte_quimicos_proveedor($fecha_inicial,$fecha_final);
}
...
<script>
	$(document).ready(function(){
	    ...
	    <?php if (count($PROVEEDORES) && isset($fecha_final))
	    {
	    	?>
		...
		$( "#excel" ).click(function() {
			...
			if (error != 1) {
				...
				var url = "mod_reportes/funciones/rep_reporte_quimicos_vendedor_excel.php?x1="+x1+"&x2="+x2+"&x3="+x3;
		...
		$('#buscar').click(function(event) {
	        ...
	        if (error == 0)
	        {
	            ...
	            $('#modal_loading').on('hidden.bs.modal', function () {
	                $.ajax({
	                	url: 'mod_reportes/ui/rep_reporte_quimicos_vendedor.php',
	                	type: 'POST',
	                	dataType: 'html',
	                	data: {
...
<div class="panel panel-default">
  	<div class="panel-body">
        <div class="page-header">
            <h1 class="translate" data-traducir_english="Chemicals By Vendor Report" data-traducir_spanish="Reporte de Químicos por Proveedor">Reporte de Químicos por Proveedor</h1>
        </div>
		<div class="row">
			<div class="col-md-3">
						...
						<table class="table table-responsive table-condensed table-striped" id="dev-table">
							<thead>
								<tr class="active info">
									...
									<th width="10%" class="translate" data-traducir_english="Company" data-traducir_spanish="Compañia">Compañia</th>
									<th width="10%" class="translate" data-traducir_english="Contact" data-traducir_spanish="Contacto">Contacto</th>
									<th width="10%" class="translate" data-traducir_english="Contact Email" data-traducir_spanish="Correo Contacto">Correo Contacto</th>
									<th width="15%" class="translate" data-traducir_english="Chemical Type" data-traducir_spanish="Tipo de Químico">Tipo de Químico</th>
									...
									<th width="10%" class="translate" data-traducir_english="Price" data-traducir_spanish="Precio">Precio</th>
	                            ...
	                            <?PHP
	                            	...
	                            	if (count($PROVEEDORES)) {
	                            		...
	                            		foreach ($PROVEEDORES as $proveedor) {
	                            			$date_insert = DateTime::createFromFormat('m-d-Y H:i:s', $proveedor['date_insert']);
	                            			$fecha = $date_insert->format('m-d-Y');
	                            			$hora = $date_insert->format('h:i:sa');
	                            			$cuerpo_tabla .= '<tr>
	                            									<td>'.$correlativo.'</td>
	                            									<td>'.utf8_encode($proveedor['finca']).'</td>
	                            									<td>'.utf8_encode($proveedor['nombre_empresa']).'</td>
	                            									<td>'.utf8_encode($proveedor['nombre_contacto']).'</td>
	                            									<td>'.utf8_encode($proveedor['correo_contacto']).'</td>
	                            									<td>'.utf8_encode($proveedor['tipo_quimicos']).'</td>
	                            									<td>'.utf8_encode($proveedor['quimicos']).'</td>
	                            									<td>'.utf8_encode($proveedor['unidades_medida']).'</td>
	                            									<td>'.utf8_encode($proveedor['precios']).'</td>
	                            									<td>'.$fecha.'</td>
	                            									<td>'.$hora.'</td>'
	                            							.'</tr>';
	                       					$correlativo++;
...
<script type="text/javascript">
	...
	/*if (count($PROVEEDORES)) {
		...
		Highcharts.chart('div_grafico', {
		    ...
		    tooltip: {
		        pointFormat: 'Products: <b>{point.y:.1f} products</b>'
		    },
		    series: [{
		        name: 'Planting',
		        data: [
		        	<?php
		        	foreach ($PROVEEDORES as $proveedor) {
		        		echo '["'.utf8_encode($proveedor['nombre_empresa'].'-'.$proveedor['nombre_contacto']).'",'.$proveedor['num_productos'].'],';
		        	}
		        	?>
		        ],
		        dataLabels: {
