	...
	$(document).ready(function(){
		...
		$( "#excel" ).click(function() {
			...
			if (error != 1) {
				...
				var url = "mod_reportes/funciones/rep_reporte_inventario_excel.php?x1="+x1+"&x2="+x2;
		...
		$('#buscar').click(function(event) {
	        ...
	        if (error == 0) 
	        {
	            ...
	            $('#modal_loading').on('hidden.bs.modal', function () {            
	                $.ajax({
	                	url: 'mod_reportes/ui/rep_reporte_inventario.php',
	                	type: 'POST',
	                	dataType: 'html',
	                	data: {
...
<div class="panel panel-default">
  	<div class="panel-body">
        <div class="page-header">
            <h1 class="translate" data-traducir_english="Inventories Report" data-traducir_spanish="Reporte de Inventarios">Reporte de Inventarios</h1>
        </div>
		<div class="row">
			<div class="col-md-4">
	                            	...
	                            	if (count($PLANTACIONES)) {
	                            		...
	                            		foreach ($PLANTACIONES as $plantacion) {
	                            			$cuerpo_tabla .= '<tr>
	                            									...
	                            									<td>'.floatval($plantacion['cantidad_quimico']+$plantacion['cantidad_enviada']+$plantacion['cantidad_sugerida']-$plantacion['cantidad_orden_compra']).'</td>
	                            									<td>'.floatval($plantacion['cantidad_aplicada']).'</td>
	                            									<td>'.floatval($plantacion['cantidad_orden_compra']).'</td>
	                            									<td>'.floatval($plantacion['cantidad_recibida']).'</td>
	                            									<td>'.floatval($plantacion['cantidad_enviada']).'</td>
	                            									<td>'.floatval($plantacion['cantidad_fisica_quimico']).'</td>
	                            									<td>'.floatval($plantacion['cantidad_quimico']+$plantacion['cantidad_enviada']+$plantacion['cantidad_sugerida']-$plantacion['cantidad_aplicada']).'</td>
	                            									<td>'.abs(floatval($plantacion['cantidad_quimico']+$plantacion['cantidad_enviada']+$plantacion['cantidad_sugerida']-$plantacion['cantidad_aplicada']-$plantacion['cantidad_fisica_quimico'])).'</td>'
	                            							.'</tr>';
	                       					$correlativo++;
	                            		}
