...
if (isset($fecha_final)) {
	//$PLANTACIONES = $DB_REP->rep_reporte_calibracion($fecha_inicial,$fecha_final,$cod_granjas_usuario);
	$PLANTACIONES = $DB_REP->rep_reporte_calibracion($fecha_inicial,$fecha_final,$cod_info_empresa);
}
	...
	$(document).ready(function(){
		...
		$( "#excel" ).click(function() {
			...
			if (error != 1) {
				...
				var url = "mod_reportes/funciones/rep_reporte_calibracion_excel.php?x1="+x1+"&x2="+x2+"&x3="+x3;
		...
		$('#buscar').click(function(event) {
	        ...
	        if (error == 0)
	        {
	            ...
	            $('#modal_loading').on('hidden.bs.modal', function () {
	                $.ajax({
	                	url: 'mod_reportes/ui/rep_reporte_calibracion.php',
	                	type: 'POST',
	                	dataType: 'html',
	                	data: {
...
<div class="panel panel-default">
  	<div class="panel-body">
        <div class="page-header">
            <h1 class="translate" data-traducir_english="Calibration Report" data-traducir_spanish="Reporte de Calibración">Reporte de Calibración</h1>
        </div>
		<div class="row">
			<div class="col-md-3">
						...
						<table class="table table-responsive table-condensed table-striped" id="dev-table">
							<thead>
								<tr class="active info">
									...
									<th width="10%" class="translate" data-traducir_english="Fertilizier" data-traducir_spanish="Fertilizante">Fertilizante</th>
									<th width="10%" class="translate" data-traducir_english="Date" data-traducir_spanish="Fecha">Fecha</th>
									<th width="15%" class="translate" data-traducir_english="Application Hours" data-traducir_spanish="Horas de Aplicación">Horas de Aplicación</th>
	                            	...
	                            	if (count($PLANTACIONES)) {
	                            		...
	                            		foreach ($PLANTACIONES as $plantacion) {
	                            			...
	                            			$cuerpo_tabla .= '<tr>
	                            									...
	                            									<td>'.utf8_encode($fertilizantes[$plantacion['cod_fertilizante']]).'</td>
	                            									<td>'.utf8_encode($plantacion['fecha_calibracion']).'</td>
	                            									<td>'.utf8_encode($plantacion['horas_aplicacion_calibrar']).'</td>
	                            									<td>'.utf8_encode($plantacion['observaciones_calibrar']).'</td>
	                            									<td>'.$fecha.'</td>
	                            									<td>'.$hora.'</td>'
	                            							.'</tr>';
	                       					$correlativo++;
...
<script type="text/javascript">
	...
	/*if (count($PLANTACIONES)) {
		...
		Highcharts.chart('div_grafico', {
		    ...
		    subtitle: {
		        text: 'Planting List with Calibrations'
		    },
		    xAxis: {
		        type: 'category',
		        labels: {
		    ...
		    tooltip: {
		        pointFormat: 'Calibrations: <b>{point.y:.1f}</b>'
		    },
		    series: [{
		        name: 'Planting',
		        data: [
		        	<?php
		        	foreach ($PLANTACIONES as $plantacion) {
		        		echo '["'.utf8_encode($plantacion['anio_plantacion'].'-'.$plantacion['num_plantacion'].'-'.$plantacion['codigo_temporada']).'",'.$plantacion['total_calibraciones'].'],';
		        	}
		        	?>
		        ],
		        dataLabels: {
