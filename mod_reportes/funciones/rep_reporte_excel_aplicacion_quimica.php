...
$cod_usuario       = trim($_GET['x1']);
$fecha_inicial     = trim($_GET['x2']);
$fecha_final       = trim($_GET['x3']);
$cod_granjas_usuario = str_replace(['[','"',']'], '', $_SESSION['cod_info_empresa']);

$PLANTACIONES = $DB_REP->rep_reporte_excel_aplicacion_quimica($cod_usuario,$fecha_inicial,$fecha_final,$cod_granjas_usuario);
/* EXCEL */
...
$objPHPExcel->setActiveSheetIndex(0)
    ->setCellValue('B2', 'BW Farming: Chemical Application Excel Download');
// Titulos de columnas
...
header('Content-Disposition: attachment;filename="Chemical Application Excel Download.xlsx"');
