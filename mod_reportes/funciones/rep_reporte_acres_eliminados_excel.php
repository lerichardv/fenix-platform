...
//$PLANTACIONES = $DB_REP->rep_reporte_acres_eliminados($fecha_inicial,$fecha_final,$cod_granjas_usuario);
$PLANTACIONES = $DB_REP->rep_reporte_acres_eliminados($fecha_inicial,$fecha_final,$cod_info_empresa);
/* EXCEL */
...
$objPHPExcel->setActiveSheetIndex(0)
    ...
    ->setCellValue('E3', 'Crop #')
    ->setCellValue('F3', 'Zones')
    ->setCellValue('G3', 'Blocks')
    ->setCellValue('H3', 'Acres')
    ->setCellValue('I3', 'Reasons')
    ->setCellValue('J3', 'System Date')
    ->setCellValue('K3', 'System Time');
...
$objPHPExcel->setActiveSheetIndex(0)
    ->setCellValue('B2', 'BW Farming: Acres Eliminated Report from ' . $fecha_inicial . ' to ' . $fecha_final);
// Titulos de columnas
...
if (count($PLANTACIONES) > 0) {
    ...
    foreach ($PLANTACIONES as $plantacion) {
        ...
        $objPHPExcel->setActiveSheetIndex(0)
            ...
            ->setCellValue('E' . $fila, utf8_encode($plantacion['crop_number']))
            ->setCellValue('F' . $fila, $wizard->toRichTextObject(utf8_decode(trim(str_replace('</br>', '<br>', utf8_encode(trim($plantacion['zonas'])))))))
            ->setCellValue('G' . $fila, $wizard->toRichTextObject(utf8_decode(trim(str_replace('</br>', '<br>', utf8_encode(trim($plantacion['bloques'])))))))
            ->setCellValue('H' . $fila, $wizard->toRichTextObject(utf8_decode(trim(str_replace('</br>', '<br>', utf8_encode(trim($plantacion['acres'])))))))
            ->setCellValue('I' . $fila, $wizard->toRichTextObject(utf8_decode(trim(str_replace('</br>', '<br>', utf8_encode(trim($plantacion['razones'])))))))
            ->setCellValue('J' . $fila, $fecha)
            ->setCellValue('K' . $fila, $hora);
        $fila += 1;
        $correlativo += 1;
    }
}
...
// Rename worksheet
$objPHPExcel->getActiveSheet()->setTitle('Acres Eliminated Report');
// Set active sheet index to the first sheet, so Excel opens this as the first sheet
$objPHPExcel->setActiveSheetIndex(0);
// Redirect output to a client’s web browser (Excel2007)
header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
header('Content-Disposition: attachment;filename="Acres Eliminated Report.xlsx"');
header('Cache-Control: max-age=0');
// If you're serving to IE 9, then the following may be needed
header('Cache-Control: max-age=1');
// If you're serving to IE over SSL, then the following may be needed
header('Expires: Mon, 26 Jul 1997 05:00:00 GMT'); // Date in the past
header('Last-Modified: ' . gmdate('D, d M Y H:K:s') . ' GMT'); // always modified
