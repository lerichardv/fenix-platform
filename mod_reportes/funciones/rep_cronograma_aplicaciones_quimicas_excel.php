...
$objPHPExcel->setActiveSheetIndex(0)
    ...
    ->setCellValue('G3', 'Application Date');
...
$objPHPExcel->setActiveSheetIndex(0)
    ->setCellValue('B2', 'BW Farming: Chemicals Aplications Chronogram from ' . $fecha_inicial . ' to ' . $fecha_final);
// Titulos de columnas
...
if (count($PLANTACIONES) > 0) {
    ...
    foreach ($PLANTACIONES as $plantacion) {
        ...
        $objPHPExcel->setActiveSheetIndex(0)
            ...
            ->setCellValue('G' . $fila, utf8_encode($plantacion['fecha_aplicacion_supervisor']));
        $fila += 1;
        $correlativo += 1;
    }
}
...
// Rename worksheet
$objPHPExcel->getActiveSheet()->setTitle('Chemicals Appli Chronogram');
// Set active sheet index to the first sheet, so Excel opens this as the first sheet
$objPHPExcel->setActiveSheetIndex(0);
// Redirect output to a client’s web browser (Excel2007)
header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
header('Content-Disposition: attachment;filename="Chemicals Aplications Chronogram.xlsx"');
