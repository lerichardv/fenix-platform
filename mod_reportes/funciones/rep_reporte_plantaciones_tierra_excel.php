...
$objPHPExcel->setActiveSheetIndex(0)
    ...
    ->setCellValue('J3', 'Blocks Range')
    ->setCellValue('K3', 'Acres')
    ->setCellValue('L3', 'Total Acres')
    ->setCellValue('M3', 'Seeds')
    ->setCellValue('N3', 'Amount Seed')
    ->setCellValue('O3', 'Lot No.')
    ->setCellValue('P3', 'Machinery Code')
    ->setCellValue('Q3', 'User Name')
...
$objPHPExcel->setActiveSheetIndex(0)
    ->setCellValue('B2', 'BW Farming: Planting Records Land Crops Report from ' . $fecha_inicial . ' to ' . $fecha_final);
// Titulos de columnas
...
if (count($PLANTACIONES) > 0) {
    ...
    foreach ($PLANTACIONES as $plantacion) {
        ...
        $objPHPExcel->setActiveSheetIndex(0)
            ...
            ->setCellValue('C' . $fila, utf8_encode($plantacion['gerencia']))
            ...
            ->setCellValue('J' . $fila, utf8_encode($plantacion['min_bloque'].'-'.$plantacion['max_bloque']))
            ->setCellValue('K' . $fila, $wizard->toRichTextObject(utf8_decode(trim(str_replace('</br>', '<br>', utf8_encode(trim($plantacion['acres'])))))))
            ->setCellValue('L' . $fila, utf8_encode($plantacion['total_acres']))
            ->setCellValue('M' . $fila, $wizard->toRichTextObject(utf8_decode(trim(str_replace('</br>', '<br>', utf8_encode(trim($plantacion['semillas'])))))))
            ->setCellValue('N' . $fila, $wizard->toRichTextObject(utf8_decode(trim(str_replace('</br>', '<br>', utf8_encode(trim($plantacion['cantidad_semillas'])))))))
            ->setCellValue('O' . $fila, $wizard->toRichTextObject(utf8_decode(trim(str_replace('</br>', '<br>', utf8_encode(trim($plantacion['lotes'])))))))
            ->setCellValue('P' . $fila, $wizard->toRichTextObject(utf8_decode(trim(str_replace('</br>', '<br>', utf8_encode(trim($plantacion['codigos_maquinaria'])))))))
            ->setCellValue('Q' . $fila, utf8_encode($plantacion['nombre_usuario']))
...
$worksheet->getStyle('L4:L'.$fila)->getNumberFormat()->setFormatCode('#,##0.00');


// Rename worksheet
$objPHPExcel->getActiveSheet()->setTitle('Planting Records Land Crops');
// Set active sheet index to the first sheet, so Excel opens this as the first sheet
$objPHPExcel->setActiveSheetIndex(0);
// Redirect output to a client’s web browser (Excel2007)
header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
header('Content-Disposition: attachment;filename="Planting Records Land Crops Report.xlsx"');
