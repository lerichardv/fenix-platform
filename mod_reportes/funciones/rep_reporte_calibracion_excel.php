...
$objPHPExcel->setActiveSheetIndex(0)
    ...
    ->setCellValue('G3', 'Fertilizier')
    ->setCellValue('H3', 'Date')
    ->setCellValue('I3', 'Application Hours')
    ->setCellValue('J3', 'Observations')
    ->setCellValue('K3', 'System Date')
    ->setCellValue('L3', 'System Time');
...
$objPHPExcel->setActiveSheetIndex(0)
    ->setCellValue('B2', 'BW Farming: Calibration Report from ' . $fecha_inicial . ' to ' . $fecha_final);
// Titulos de columnas
...
if (count($PLANTACIONES) > 0) {
    ...
    foreach ($PLANTACIONES as $plantacion) {
        ...
        $objPHPExcel->setActiveSheetIndex(0)
            ...
            ->setCellValue('G' . $fila, $wizard->toRichTextObject(utf8_decode(trim(str_replace('</br>', '<br>', utf8_encode(trim($fertilizantes[$plantacion['cod_fertilizante']])))))))
            ->setCellValue('H' . $fila, utf8_encode($plantacion['fecha_calibracion']))
            ->setCellValue('I' . $fila, $wizard->toRichTextObject(utf8_decode(trim(str_replace('</br>', '<br>', utf8_encode(trim($plantacion['horas_aplicacion_calibrar'])))))))
            ->setCellValue('J' . $fila, $wizard->toRichTextObject(utf8_decode(trim(str_replace('</br>', '<br>', utf8_encode(trim($plantacion['observaciones_calibrar'])))))))
            ->setCellValue('K' . $fila, $fecha)
            ->setCellValue('L' . $fila, $hora);
        $fila += 1;
        $correlativo += 1;
    }
}
...
// Rename worksheet
$objPHPExcel->getActiveSheet()->setTitle('Calibration Report');
// Set active sheet index to the first sheet, so Excel opens this as the first sheet
$objPHPExcel->setActiveSheetIndex(0);
// Redirect output to a client’s web browser (Excel2007)
header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
header('Content-Disposition: attachment;filename="Calibration Report.xlsx"');
header('Cache-Control: max-age=0');
// If you're serving to IE 9, then the following may be needed
header('Cache-Control: max-age=1');
// If you're serving to IE over SSL, then the following may be needed
header('Expires: Mon, 26 Jul 1997 05:00:00 GMT'); // Date in the past
header('Last-Modified: ' . gmdate('D, d M Y H:L:s') . ' GMT'); // always modified
