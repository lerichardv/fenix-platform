...
$objPHPExcel->setActiveSheetIndex(0)
            ->setCellValue('B1', 'Ingresos al Sistema OMNIABOX del '.$fecha_inicial.' al '.$fecha_final);
// Titulo principal
...
$objPHPExcel->setActiveSheetIndex(0)
            ->setCellValue('B2', 'Listado de ingresos al sistema mensual');
// Titulos de columnas
...
if(count($TENDENCIAS) > 0){
    foreach($TENDENCIAS as $INFO)
    {
        ...
        ->setCellValue('E'.$fila, (utf8_encode($INFO['ingresos'])));
...
header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet; charset=utf-8; format=attachment;');
header('Content-Disposition: attachment;filename="Reporte de ingresos a OMNIABOX Mensuales '.date("Y-m-d").'.xlsx"');
