...
$DB_PERFILES = $DB_USUARIO->get_listado_perfiles();
?>
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        /* Checkboxs */
        $( ".checkbox_perfil" ).on( "click", function() {
            flag_check   = 0;
            flag_ins_act = 0;
    ...
    function insertar_registro(x1, x2){
        ...
        $.ajax({
                type: 'POST',
                url: 'mod_admin_usuarios/funciones/usu_insertar_perfil.php',
                beforeSend: function () {
                    if (x1 == '' || x2 == ''){
                ...
                success: function (data) {
                    ...
                    $("#modal_loading").modal("hide");
                    flag_ins_act = 0;
                    grl_obtener_cuerpo_menu(2, 'mod_admin_usuarios/ui/usu_perfiles.php');
                }
        }); //Ajax
        jQuery.ajaxSetup({async: true});
    }
    ...
    function actualizar_registro(x1, x2, x3, x4){
        ...
        $.ajax({
                type: 'POST',
                url: 'mod_admin_usuarios/funciones/usu_actualizar_perfil.php',
                beforeSend: function () {
                    if (flag_ins_act != 0){
...
<style type="text/css">
    ...
    .material-switch > .checkbox_perfil:checked + label::before {
        background: inherit;
        opacity: 0.5;
    }
    .material-switch > .checkbox_perfil:checked + label::after {
        background: inherit;
        left: 20px;
    }
    ...
    <div class="panel panel-default">
        <div class="panel-body" >
            <div class="page-header">
                <h1 class="translate" data-traducir_english="Users profiles administration" data-traducir_spanish="Administración de perfiles para usuarios">Administración de perfiles para usuarios</h1>
            </div>
            <div class="row">
                <div class="col-md-3">
                                    ...
                                    <label for="nombre_registro" class="translate" data-traducir_english="Profile name" data-traducir_spanish="Nombre del perfil">Nombre del perfil</label>
                                    <input type="text" class="form-control" placeholder="Nombre perfil" id="nombre_registro">
                                </div>
                                            </div>
                                            <div class="col-md-12">
                                                <label for="iniciales_registro" class="translate" data-traducir_english="Profile description" data-traducir_spanish="Descripción del perfil">Descripción del perfil</label>
                                                    <div class="input-group input-group-sm">
                                                  <input type="text" class="form-control" placeholder="Descripción del perfil" id="iniciales_registro">
                ...
                <div class="col-md-9">
                    <div class="panel panel-primary">
                        <!-- Titulo del panel -->
                        <div class="panel-heading translate" data-traducir_english="Registered profiles list" data-traducir_spanish="Listado de perfiles registrados">Listado de perfiles registrados</div>

                        <!-- listado -->
                        <ul class="list-group" id="listado_items">
                            <?PHP
$cuerpo_lista = '';
if (count($DB_PERFILES) > 0) {
    $check = 'checked';
    foreach ($DB_PERFILES as $INFO) {
        $check = ($INFO['activo'] == 1) ? 'checked' : '';
        $cuerpo_lista .= '
                                            <li class="list-group-item" style="padding-bottom:20px">
                                                <label class="registro">' . utf8_encode($INFO['perfil']) . ' <small>' . utf8_encode($INFO['descripcion']) . '</small></label>
                                                <div class="material-switch pull-right">
                                                    <input class="checkbox_perfil" id="' . utf8_encode($INFO['cod_perfil']) . '" name="' . utf8_encode($INFO['cod_perfil']) . '" value="' . utf8_encode($INFO['perfil']) . '" initials="' . utf8_encode($INFO['descripcion']) . '" type="checkbox" ' . $check . '/>
                                                    <label for="' . utf8_encode($INFO['cod_perfil']) . '" class="label-primary"></label>
                                                </div>
                                            </li>';
    }
} else {
    //Si no hay registros informará al usuario
