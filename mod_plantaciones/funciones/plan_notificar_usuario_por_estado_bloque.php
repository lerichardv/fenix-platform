<?php
/*
 * Envía un correo eletrónico a cada usuario de acuerdo al estado de plantación
...
$cod_estado_plantacion  = $_POST['x1'];
$codigo_plantacion      = $_POST['x2'];
$codigo_bloque          = $_POST['x3'];

$USUARIOS = $DB_PLAN->plan_listado_usuarios_notificar_por_estado_plantacion($cod_estado_plantacion);

$USUARIO = $DB_USUARIO->get_info_usuario_by_cod_usuario($_SESSION['cod_usuario']);

$PLANTACION = $DB_PLAN->plan_obtener_info_plantacion($codigo_plantacion);
$BLOQUE = $DB_PLAN->bw_obtener_info_bloque($codigo_bloque);
                                                    ...
                                                    <tbody>
                                                        <tr>
                                                            <td style="word-wrap:break-word;font-size:0px;padding:19px 20px 19px 20px;" align="left">
                                                                <div style="cursor:auto;color:#000000;font-family:Cabin, sans-serif;font-size:15px;line-height:22px;text-align:left;"><h2 style="color: #F05D22; line-height: 100%;"><span style="color:#16a085;">Planting Block change of stage</span></h2>
                                                                    <p>The Block '.$BLOQUE[0]['zona'].'-'.$BLOQUE[0]['nombre_bloque'].' of planting '.$PLANTACION[0]['anio_plantacion'].'-'.$PLANTACION[0]['num_plantacion'].'-'.$PLANTACION[0]['codigo_temporada'].' has been passed to the stage '.$USUARIOS[0]['estado_plantacion_english'].' by the user '.$USUARIO[0]['nombre'].'. Please go to B&W Farming to check it.</p>
                                                                </div>
                                                            </td>
                                                        </tr>
                                                        <tr>
...
$mail->Subject = 'Notification - Block '.$BLOQUE[0]['zona'].'-'.$BLOQUE[0]['nombre_bloque'].' of Planting '.$PLANTACION[0]['anio_plantacion'].'-'.$PLANTACION[0]['num_plantacion'].'-'.$PLANTACION[0]['codigo_temporada'].' change of stage';
