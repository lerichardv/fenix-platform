<?php
/*
* 	Administración de todos los campos que se usaran en las plantaciones de las granjas
...
$CAMPOS = $DB_INV->inv_listado_campos();

$codigo_field = $_POST['cod_field'];
if (!isset($_POST['cod_field'])) {
	$codigo_field = 0;
}

$GRUPO = $DB_INV->inv_obtener_campo($codigo_field);
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		if (count($GRUPO)) {
		?>
			$('#nombre_campo').val("<?php echo utf8_encode(($GRUPO[0]['field'])); ?>");
			$('#cod_granja').val("<?php echo $GRUPO[0]['cod_farm']; ?>");
		...
		?>
		codigo_field = <?php echo $codigo_field; ?>;
	...
	$('#btn_guardar').click(function(event) {
		...
		if (error == 0) {
			...
			$('#modal_loading').on('hidden.bs.modal', function() {
				inv_guardar_campo(codigo_field);
			});
		} else {
	...
	$('.checkbox').change(function(event) {
		...
		inv_cambiar_estado_campo($(this).data('id'), ($(this).attr('checked') ? 0 : 1));
	});
</script>
		...
		<div class="panel-body">
			<div class="col-md-12">
				<div class="panel-header">
					<h1 class="translate" data-traducir_english="Fields" data-traducir_spanish="Campos">Fields</h1>
				</div>
			</div>
			<div class="col-md-12">
				<div class="row">
					...
					<div class="col-md-3">
						<div class="form-group input-group-sm" id="div_nombre_capacitacion">
							<label for="nombre_campo" class="translate" data-traducir_english="Field" data-traducir_spanish="Campo">Field</label>
							<input type="text" class="form-control letras input requerido requerido_nueva_capacitacion" id="nombre_campo" name="nombre_campo">
						</div>
					</div>
			...
			<div class="col-sm-12">
				<div class="panel panel-primary" style="overflow:auto;">
					<div class="panel-heading panel_cabecera">
						<h3 class="panel-title translate" data-traducir_english="Fields List" data-traducir_spanish="Lista de campos">Fields List</h3>
							...
							<tbody id="cuerpo_tabla">
								...
								?>
										<tr class="fila_seleccionable">
											<td onclick="inv_vista_campo(<?php echo utf8_encode($campo['cod_field']); ?>)"><?php echo $correlativo; ?></td>
											<td onclick="inv_vista_campo(<?php echo utf8_encode($campo['cod_field']); ?>)"><?php echo utf8_encode($campo['nombre_granja']); ?></td>
											<td onclick="inv_vista_campo(<?php echo utf8_encode($campo['cod_field']); ?>)"><?php echo utf8_encode($campo['field']); ?></td>
											<td>
												<div class="material-switch pull-right">
													<input class="checkbox" id="checkbox_<?php echo utf8_encode($campo['cod_field']); ?>" data-id="<?php echo utf8_encode($campo['cod_field']); ?>" name="checkbox_<?php echo utf8_encode($campo['cod_field']); ?>" type="checkbox" <?php echo ($campo['activo'] == 1 ? 'checked="checked"' : ''); ?> />
													<label for="checkbox_<?php echo utf8_encode($campo['cod_field']); ?>" class=""></label>
												</div>
											</td>
										</tr>
								<?php
