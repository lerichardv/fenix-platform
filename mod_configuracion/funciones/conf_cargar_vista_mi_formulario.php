            		...
            		foreach ($ITEMS as $item) {
        				switch ($item['cod_tipo_item'])
        				{
        					...
        					case '3': /*ITEM ES UN SELECTPICKER*/
								...
								<script type="text/javascript">
									$('#<?php echo utf8_encode($item['id_item']);?>').selectpicker('val',"<?php echo $item['observacion']; ?>");
									$('.selectpicker').selectpicker('refresh');
								</script>
        						<?php
        						break;
        					...
        					case '9': //radiobutton
								...
								<script type="text/javascript">
									$('input[name="<?php echo utf8_encode($item['id_item']);?>"][value="<?php echo $item['observacion']; ?>"]').prop('checked', true);
								</script>
        						<?php
        						break;
        					default:
