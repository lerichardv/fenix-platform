<?php
/*
 * Envía un correo eletrónico al jefe inmediato del usuario que ingresó el formulario
...
$codigo_detalle = $_POST['x2'];
$prioridad      = $_POST['x3'];

$high_priority = '';
if($prioridad == 1) $high_priority = 'High Priority - ';


$FORMULARIO     = $DB_CONF->conf_obtener_info_formulario_sa($cod_formulario);
$USUARIO        = $DB_USU->get_info_usuario_by_codigo($_SESSION['cod_usuario']);


$tabla = '<table class="table table-responsive table-striped" width="100%">'.
                '<thead>'.
                    '<th width="20%">No.</th>'.
                    '<th width="40%">Item</th>'.
...
//$ITEMS  = $DB_CONF->conf_obtener_items_activos_formulario_sa($FORMULARIO[0]['cod_formulario']);
$ITEMS  = $DB_CONF->conf_obtener_respuestas_items_activos_formulario_sa($codigo_detalle);
if (count($ITEMS) > 0)
{
	$correlativo = 1;
    foreach ($ITEMS as $item)
    {
        if ($item['flag_alerta'] == 1 && $item['observacion'] != null)
        {
            $tabla .= '<tr><td>'.$correlativo.'</td><td>'.$item['nombre_item'].'</td><td>'.($item['texto_detalle'] != null ? $item['texto_detalle']:$item['observacion']).'</td></tr>';
            $correlativo++;
        }
    }
}
                                        ...
                                        <div style="border-top:0px solid transparent; border-left:0px solid transparent; border-bottom:0px solid transparent; border-right:0px solid transparent; padding-top:0px; padding-bottom:5px; padding-right: 0px; padding-left: 0px;">
                                            ...
                                            <div style="color:#0D0D0D;font-family:Montserrat, Trebuchet MS, Lucida Grande, Lucida Sans Unicode, Lucida Sans, Tahoma, sans-serif;line-height:1.2;padding-top:10px;padding-right:10px;padding-bottom:10px;padding-left:10px;">
                                                <div style="font-size: 12px; line-height: 1.2; color: #0D0D0D; font-family: Montserrat, Trebuchet MS, Lucida Grande, Lucida Sans Unicode, Lucida Sans, Tahoma, sans-serif; mso-line-height-alt: 14px;">
                                                    <p style="font-size: 28px; line-height: 1.2; text-align: center; word-break: break-word; mso-line-height-alt: 34px; margin: 0;"><span style="font-size: 28px;"><strong><span style="font-size: 28px;">Hello Dear User,</span></strong></span><br /><span style="font-size: 28px;">A new Form "'.$FORMULARIO[0]['nombre_formulario'].'" has been filled by '.utf8_encode($USUARIO[0]['nombre']).'.<br /></span></p>
                                                </div>
                                            </div>
                                            <!--[if mso]></td></tr></table><![endif]-->
                                            ...
                                            <div style="color:#555555;font-family:Montserrat, Trebuchet MS, Lucida Grande, Lucida Sans Unicode, Lucida Sans, Tahoma, sans-serif;line-height:1.5;padding-top:10px;padding-right:10px;padding-bottom:10px;padding-left:10px;">
                                                <div style="font-size: 12px; line-height: 1.5; color: #555555; font-family: Montserrat, Trebuchet MS, Lucida Grande, Lucida Sans Unicode, Lucida Sans, Tahoma, sans-serif; mso-line-height-alt: 18px;">
                                                    <p style="font-size: 14px; line-height: 1.5; text-align: center; word-break: break-word; mso-line-height-alt: 21px; margin: 0;">'.$tabla.'<span style="color: #a8bf6f; font-size: 14px;"><strong><br /></strong></span></p>
                                                </div>
                                            </div>
                                            <!--[if mso]></td></tr></table><![endif]-->
...
//{
$mail->AddAddress($USUARIO[0]['email_jefe'], utf8_encode($USUARIO[0]['nombre_jefe']));
$mail->AddAddress($USUARIO[0]['email'], utf8_encode($USUARIO[0]['nombre']));
//}

$mail->isHTML(true);                                  // Set email format to HTML
$mail->Subject = $high_priority.'Notification - Form '.$FORMULARIO[0]['nombre_formulario'].' has been filled';
